...
class Segment:
    ...
    def length(self): # формула нахождения длины отрезка по координатам, сразу округляем
        return round(((((self.x2 - self.x1) ** 2) + (self.y2 - self.y1) ** 2) ** 0.5), 2)

    def x_axis_intersection(self): # если произведение координат Y меньше 0, то отрезок пересекает ось абсцисс
        if self.y1 * self.y2 < 0:
            return True
        else:
            return False

    def y_axis_intersection(self): # если произведение координат X меньше 0, то отрезок пересекает ось ординат

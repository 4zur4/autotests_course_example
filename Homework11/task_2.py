...
try:
    ...
    new_message.click()
    sleep(2)
    search_recipient = driver.find_element(By.CSS_SELECTOR, ".controls-StackTemplate__top-area-content " ".controls-Search__nativeField_caretEmpty")
    search_recipient.send_keys(recipient)
    sleep(2)
    find_recipient = driver.find_element(By.CSS_SELECTOR, ".msg-addressee-item [title='Нарядный Человек']")
    ...
    delete_btn.click()
    sleep(3)
    assert driver.find_elements(By.CSS_SELECTOR, '.msg-dialogs-item p')[0] != message, 'Сообщение не удалено'
    sleep(2)
finally:
    driver.quit()
